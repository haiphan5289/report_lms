...
final class GroupInspectionsByWeekUseCase {
    ...
    func execute(_ inspections: [Inspection]) -> [WeekSection] {
        ...
        let sections = weekGroups.compactMap { key, inspectionList -> WeekSection? in
            ...
            let weekStartDate = calendar.dateInterval(of: .weekOfYear, for: firstInspection.createdAt)?.start ?? firstInspection.createdAt
            let sortedInspections = inspectionList.sorted { $0.createdAt > $1.createdAt }
