...
User taps "Gửi Email" in FinalReportView (uploads still running)
  → isUploading = (activeUploadCount > 0) → button shows spinner + totalUploadingImageCount
  → tapping while isUploading → showUploadStatusSheet = true + pendingEmailSend = true
  → UploadStatusBottomSheet shows live progress per image
  → upload completes → onTaskCompleted → notifyUploadCompleted → activeUploadCount → 0
  → FinalReportView.onChange(activeUploadCount) → sendReport() auto-trigger
  → sendReportViaQueue → report_delivery_queue doc → markInspectionCompleted()
     └── reads fresh from storageService.getInspection(by: id) (not stale self.inspection) ✅
```
...
### 🐛 BUG-012 — PDF Firebase không có ảnh: auto-send email trước khi Firestore imageURLs được write

**Date:** 2026-06-09
**Branch:** `feat/login`
**Severity:** High (PDF report missing all photos)

#### Root Causes (2 độc lập, cùng gây ra symptom)

**1. B5 — `onChange(totalUploadingImageCount)` fires trước Firestore write**

`totalUploadingImageCount` về 0 khi per-image `doneCb` hoàn tất (upload network xong). Nhưng `imageURLs` chỉ được write vào Firestore SAU đó, bên trong `storageService.updateInspection()` ở cuối `uploadPhotosAndUpdateField`. Cloud Function đọc Firestore trong window này → `field.imageURLs = []`.

**Fix (FinalReportView.swift):** Đổi sang `onChange(of: activeUploadCount)`. `activeUploadCount` về 0 chỉ sau `onTaskCompleted` → `notifyUploadCompleted()` — fire SAU Firestore write.

```swift
// Trước: .onChange(of: inspectionDetailVM.totalUploadingImageCount)
// Sau:   .onChange(of: inspectionDetailVM.activeUploadCount)
// Trước: let isUploading = inspectionDetailVM.totalUploadingImageCount > 0
// Sau:   let isUploading = inspectionDetailVM.activeUploadCount > 0
```

**2. B6 — `markInspectionCompleted` dùng stale `self.inspection` snapshot**

`FinalReportViewModel.inspection` là Swift `struct` snapshot từ khi `FinalReportView` init (trước upload). `markInspectionCompleted()` gọi `storageService.updateInspection(self.inspection)` → ghi đè Firestore với `imageURLs = []`. CF được trigger ngay trước đó bởi `report_delivery_queue` doc → đọc Firestore sau khi bị overwrite → `imageURLs = []` → PDF trống.

**Fix (FinalReportViewModel.swift):**
```swift
// Trước (BUG):
...
`storageService` là singleton — cache đã được `uploadPhotosAndUpdateField` update với URLs đúng.

#### Rule to Remember

> Khi auto-trigger action phụ thuộc vào data đã được write vào Firestore, phải dùng signal fires SAU write (`activeUploadCount`), không dùng signal fires SAU network upload (`totalUploadingImageCount`). Và không dùng `self.inspection` struct snapshot khi cần write về Firestore sau upload — luôn đọc fresh từ shared cache.
